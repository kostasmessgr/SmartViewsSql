        ...
        //contract.methods.groupId().call(function (err, result) {
            ...
            //if (!err) {
                ...
                //if (result > 0) {
                    ...
                    //console.log("before calling getStructsSQL");
                    return computationsController.getStructsSQL().then((resultGB)=> {
                        const getAllGBsTime = helper.time() - getAllGBsFromBCTimeStart;
                        //console.log('getAllGBs '+resultGB.length);
                        //console.log('1. '+typeof resultGB);
                        //console.log('2. '+JSON.stringify(resultGB));
                        //console.log('resultGB '+JSON.stringify(resultGB));
                        ...
                        //if (!err) {
                           // resultGB = removeUnneededFieldsFromBCResponse(resultGB);
                            ...
                            //const results = JSON.parse(resultGB);
                            //console.log('before resolve get allgb '+JSON.stringify(resultGB));
                            resolve(resultGB);
                        //} 
                    }).catch((e)=>{
                        helper.log(e);
...
async function getLatestId () {
    return new Promise((resolve, reject) => {
        computationsController.getLatestIdSQL(function (err, latestId) {
            if (err) {
                /* istanbul ignore next */
                console.log(err);
                /* istanbul ignore next */
...
function deleteCachedResults (sortedByEvictionCost) {
    //console.log('contract controller.deleteCached results');
    //helper.cacheNow(sortedByEvictionCost,"Evict",'in contractController');
    let i=0;
    // for(i;i<sortedByEvictionCost.length;i++){
    //     console.log(JSON.stringify(sortedByEvictionCost[i].hash));
    // }
    return new Promise((resolve, reject) => {
        cacheController.deleteFromCache(sortedByEvictionCost).then(gbIdsToDelete => {
            //helper.cacheNow(gbIdsToDelete,"Evict gbs",'in contractController');
            helper.log('IDS DELETED FROM CACHE:');
            helper.log(gbIdsToDelete);
            helper.log('*********');
            computationsController.deleteMultipleStructsSQL(gbIdsToDelete).then(receipt => {
                resolve(receipt);
            }).catch(error => {
                /* istanbul ignore next */
                helper.log(error);
