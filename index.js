...
app.get('/load_dataset_sql/:dt', contractController.contractChecker, function (req, res) {
    ...
    if (!running) {
        ...
        running = true;
        let startTime = helper.time();
        computationsController.insertIntoSQL(dt).then(retval => {
            let endTime = helper.time();
            let timeDiff = endTime - startTime;
            running = false;
            io.emit('DONE', 'TRUE');
            helper.log('Added ' + dt.length + ' records SQL in ' + timeDiff + ' seconds');
            res.send({ message: 'OK' });
        }).catch(error => {
            /* istanbul ignore next */
            helper.log(error);
    ...
    if (!gbRunning && !running) {
        ...
        viewMaterializationController.materializeView(view,
            ...
            .then(result => {
                ...
                /* istanbul ignore next */
                //console.log('error in materialize view calling from index '+err);
...
app.get('/getSqlViewByName/:viewName/:contract', contractController.contractChecker, async function (req, res) {
    if (process.env.TESTS) {
        config = reload('./config_private');
    }
    const totalStart = helper.time();
